...
const QuotaDisplay = ({ userId }) => {
    ...
    const limits = PLAN_LIMITS[plan] || PLAN_LIMITS['free']; // Fallback to free if plan unknown
    const isPlus = plan === 'plus';

    // Free Plan Only: Hide if Plus
    if (isPlus) return null;

    // Helper to format limit text
    const formatLimit = (current, max) => isPlus ? 'Unlimited' : `${current} / ${max}`;
    // Helper to calculate percentage for progress bar
